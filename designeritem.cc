...
MyDesignerItem::MyDesignerItem(QRect rect, QGraphicsScene *p) :
...
void MyDesignerItem::hoverLeaveEvent ( QGraphicsSceneHoverEvent * )
{
    ...
    _corners[0]->setParentItem(nullptr);
    _corners[1]->setParentItem(nullptr);
    _corners[2]->setParentItem(nullptr);
    _corners[3]->setParentItem(nullptr);
...
DesignerPixmapItem::DesignerPixmapItem(QPixmap &pix, QRect rect, Scene *p)
    : MyDesignerItem(rect,p)
{
    _widget = new QLabel;
    _widget->setGeometry(rect);
    reinterpret_cast<QLabel *>(_widget)->setPixmap(pix);
    _pixmap = _widget->grab();
}
